...
class LinearHashTable {
  ...
  void resize();
  int hash(T x) {
    return (unsigned) ((z * x) % (2*(1 << d))); // Why doesn't this work? Don't ask me. I bet it's a missing parenthesis.
  }
  int hash2(T x) {
    return 1 + (hash(x) % ((2*(1 << d) - 1))); // Same here. Not liable for computers catching on fire.
  }
...
public:
  ...
  template<class T>
  bool LinearHashTable<T>::addSlow(T x) { // This method == jank. 10:47, with 2 exams to study for. Should have started earlier (again).
    int both_len = t.length + s.length;
    int insert = null;
    if (2 * (q + 1) > t.length)
      resize();   // max 50% occupancy
  ...
  template<class T>
  void LinearHashTable<T>::PrintTable(int add_amt) { // For testing purposes only.
...
#endif NIHY_LINEARHASHTABLE_H_ // Can't get rid of this error, don't know why. Doesn't impact anything, just pops up a dialog box.
